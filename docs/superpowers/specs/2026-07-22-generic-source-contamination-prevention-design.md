...
- every used placeholder is declared once;
- every declared placeholder is used;
...
- definitions referenced through explicit compiler slot dependencies;
...
```bash
pnpm audit:template-contamination --dry-run
```
...
- owner;
- name/title with replacement suffix;
- topic;
- approved editorial brief content after chapter-ID remapping;
...
- Every exportable generation is checked against template-scoped source profiles.
